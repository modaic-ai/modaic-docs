...
Modaic is a hub to share and manage DSPy programs. We provide the `PrecompiledProgram` class which is a subclass of `dspy.Module` with some extra functionality for observability and version control.
...
optimized programs with DSPy, refer to the [DSPy Documentation](https://dspy.ai/).
...
You can still use Modaic without uv, but you will not be upload DSPy programs to the Modaic Hub.
...
- <b>PrecompiledProgram</b>: subclass of `dspy.Module` that implements `forward(...)`. Save and share it with `save_precompiled`/`push_to_hub`.
- <b>Retriever</b> and <b>Indexer</b>: retrieval components. `Indexer` extends `Retriever` by adding an `ingest` method.
- <b>PrecompiledConfig</b>: Configuration class for `PrecompiledProgram` and `Retriever` classes. Use this to define experiment specific parameters.
- <b>AutoProgram/AutoConfig/AutoRetriever</b>: dynamic loaders that fetch classes and configs from a local path or the Modaic hub.
...
- [Auto loaders](/reference/modaic/auto_program)
- [Precompiled Program](/reference/modaic/precompiled_program)
...
<Tip>
Your module must have an attribute `config` that is annotated with the config class. Forward references are not supported.
Example:
```python
config: EchoConfig
```
...
class EchoConfig(PrecompiledConfig):
    lm: str = "gpt-4o"
class EchoProgram(PrecompiledProgram):
...
program = EchoProgram(EchoConfig())
print(module(text="hello"))
```
<Info>
`PrecompiledProgram` is a `dspy.Module`; call your module like a function and implement `forward(...)`.
</Info>
...
loaded = AutoProgram.from_precompiled("./my-echo", local=True)
...
```python
# Requires MODAIC_TOKEN; pushes to "yourname/echo-module"
program.push_to_hub("yourname/echo-module", commit_message="initial module")

remote = AutoProgram.from_precompiled("yourname/echo-module")
